...
@php
    // desc_points can be array (if cast) or json string
    $descPoints = $booking->desc_points;

    if (is_string($descPoints)) {
        $descPoints = json_decode($descPoints, true);
    }

    if (!is_array($descPoints)) {
        $descPoints = [];
    }
@endphp
...
{{-- Pass existing desc points to JS --}}
<script>
    window.existingAirDescPoints = @json($descPoints);
</script>
                    <div class="row">
                        <div class="col-md-6 mb-3">
                            <div class="card border-secondary">
...
<script>
    ...
    function addAirDescBlock(prefill = null) {
        ...
        // Prefill title
        if (prefill && typeof prefill.title !== 'undefined') {
            const titleInput = block.querySelector(`input[name="desc_points[${idx}][title]"]`);
            if (titleInput) titleInput.value = prefill.title ?? '';
        }

        // Prefill subs (or add one empty)
        const subs = (prefill && Array.isArray(prefill.subs)) ? prefill.subs : [];
        if (subs.length) {
            subs.forEach(s => addAirSubPoint(idx, s));
        } else {
            addAirSubPoint(idx);
        }
    }
    ...
    function addAirSubPoint(idx, value = '') {
        const block = document.querySelector(`#airDescPointsWrapper [data-index="${idx}"]`);
        if (!block) return;
    ...
    document.addEventListener('DOMContentLoaded', function() {
        // Bind add button
        const addBtn = document.getElementById('addAirDescBlockBtn');
        if (addBtn) addBtn.addEventListener('click', function() {
            addAirDescBlock();
        });

        // Load existing data
        const existing = window.existingAirDescPoints || [];
        if (existing.length) {
            existing.forEach(p => addAirDescBlock(p));
        } else {
            // at least one block
            addAirDescBlock();
        }
