            ...
            <div id="invoiceContent">
                ...
                <div
                    ...
                    <div style="text-align:center; margin-bottom:30px;">
                        ...
                        <span
                            style="background:{{ $badgeColor }}; color:white; padding:5px 15px; border-radius:3px; font-size:12px; font-weight:bold;">{{ strtoupper($booking->status) }}</span>
                    </div>
                        ...
                        <tr>
                            ...
                            <td style="vertical-align: top; width:50%; padding-left:15px; text-align:right;">
                                ...
                                <p style="margin:5px 0; color:#666; font-size:14px;"><strong>Payment Status:</strong> <span
                                        style="color:{{ $badgeColor }}; font-weight:bold;">{{ strtoupper($booking->payment_status) }}</span>
                                </p>
                            </td>
                        </tr>
                    </table>
                                        ...
                                        <tr>
                                            <td
                                                ...
                                                <div style="margin-top:8px;">
                                                    @foreach ($thumbnails as $thumb)
                                                        <img src="{{ $thumb ? asset('storage/' . ltrim($thumb, '/')) : 'https://via.placeholder.com/60x45?text=No+Image' }}"
                                            ...
                                            <td style="vertical-align:top; text-align:left; padding-left:15px;">
                                                <p style="margin:0 0 10px 0; font-size:18px; color:#333; font-weight:bold;">
                                                    {{ $booking->vehicle->name }} - {{ $booking->vehicle->model }}</p>
                                                <p style="margin:8px 0; color:#666; font-size:14px;">
                                                    <strong>Pickup:</strong> {{ $booking->pickup_location }}</p>
                                                <p style="margin:8px 0; color:#666; font-size:14px;">
                                                    <strong>Date/Time:</strong>
                                                    {{ $booking->pickup_datetime?->format('Y-m-d H:i') ?? 'N/A' }}</p>
                                                <p style="margin:8px 0; color:#666; font-size:14px;">
                                                    <strong>Drop-off:</strong> {{ $booking->dropoff_location }}</p>
                                                <p style="margin:8px 0; color:#666; font-size:14px;">
                                                    <strong>Date/Time:</strong>
                                                    {{ $booking->dropoff_datetime?->format('Y-m-d H:i') ?? 'N/A' }}</p>
                                            </td>
                                        </tr>
                                    </table>
                                </td>
                        ...
                        <tbody>
                            <tr style="border-bottom:1px solid #ddd;">
                                ...
                                <td style="padding:12px; text-align:right; font-size:14px; color:#666;">
                                    {{ $booking->currency }} {{ number_format($booking->price, 2) }}</td>
                            </tr>
                            <tr style="border-bottom:1px solid #ddd;">
                                <td style="padding:12px; font-size:14px; color:#666;">Additional Charges</td>
                                <td style="padding:12px; text-align:right; font-size:14px; color:#666;">
                                    {{ $booking->currency }} {{ number_format($booking->additional_charges, 2) }}</td>
                            </tr>
                            <tr style="border-bottom:1px solid #ddd;">
                                ...
                                <td style="padding:12px; text-align:right; font-size:14px; color:#28a745;">-
                                    {{ $booking->currency }} {{ number_format($booking->discount, 2) }}</td>
                            </tr>
                            <tr style="background:#f0f0f0;">
                                ...
                                <td
                                    ...
                                    {{ $booking->currency }} {{ number_format($booking->total_price, 2) }}</td>
                            </tr>
                        </tbody>
                    </table>
                    ...
                    @if ($booking->note)
                        <div style="margin-bottom:20px; padding:15px; background:#fffbea; border-left:4px solid #ffc107;">
                            <p style="margin:0; font-size:14px; color:#666;"><strong>Note:</strong> {{ $booking->note }}
                            </p>
                        </div>
                    @endif
