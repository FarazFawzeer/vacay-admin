...
@section('content')
    ...
    <div class="card">
        ...
        <div class="card-body">
          ...
          <form action="{{ route('admin.visa-bookings.update', $booking->id) }}" method="POST" id="visaBookingForm">
    @csrf
    @method('PUT')
                <input type="hidden" id="invoice_no" value="{{ $booking->invoice_no ?? 'VB-0001' }}">
                ...
                <div class="row">
                        ...
                        <select name="visa_id" id="visa_id" class="form-select" required>
                            ...
                            @foreach ($visas as $visa)
                                <option value="{{ $visa->id }}" data-country="{{ $visa->country }}"
                                    {{ $booking->visa_id == $visa->id ? 'selected' : '' }}>
                    ...
                    <div class="col-md-6 mb-3">
                        ...
                        <input type="text" name="passport_number" id="passport_number" class="form-control"
                               value="{{ $booking->passport_number }}" required>
                    </div>
                    ...
                    <div class="col-md-6 mb-3">
                        ...
                        <input type="date" name="visa_issue_date" id="visa_issue_date" class="form-control"
                               value="{{ $booking->visa_issue_date->format('Y-m-d') }}" required>
                    </div>
                    ...
                    <div class="col-md-6 mb-3">
                        ...
                        <input type="date" name="visa_expiry_date" id="visa_expiry_date" class="form-control"
                               value="{{ $booking->visa_expiry_date->format('Y-m-d') }}" required>
                    </div>
                ...
                <div class="text-end gap-2 d-flex justify-content-end">
                    <a href="{{ route('admin.visa-bookings.index') }}" class="btn btn-light" style="width:135px;">Back</a>
                    <button type="button" class="btn btn-secondary" onclick="previewVisaBooking()"
                            style="width:135px;">Preview</button>
...
<script>
    ...
    function previewVisaBooking() {
        ...
        const badgeColors = { pending: '#ffc107', approved: '#198754', rejected: '#dc3545' };
            ...
            <tr>
                <td style="padding:20px;">
                    ...
                    <table style="width:100%;border-collapse:collapse;">
                        <tr><td style="padding:8px 0;font-size:14px;color:#666;"><strong>Visa:</strong> ${visaName}</td></tr>
                        <tr><td style="padding:8px 0;font-size:14px;color:#666;"><strong>Passport No:</strong> ${passportNumber}</td></tr>
                        <tr><td style="padding:8px 0;font-size:14px;color:#666;"><strong>Agent:</strong> ${agent}</td></tr>
                        <tr><td style="padding:8px 0;font-size:14px;color:#666;"><strong>Issue Date:</strong> ${visaIssueDate}</td></tr>
                        <tr><td style="padding:8px 0;font-size:14px;color:#666;"><strong>Expiry Date:</strong> ${visaExpiryDate}</td></tr>
                    </table>
                </td>
            </tr>
        </table>
