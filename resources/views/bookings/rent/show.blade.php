                    ...
                    <table style="width:100%; margin-bottom:30px; border-collapse: collapse;border: none;">
                        <tr>
                            <td style="vertical-align: top; width:50%; padding-right:15px;">
                                ...
                                <p style="margin:5px 0; color:#666; font-size:14px;"><strong>Name:</strong> {{ $booking->customer->name }}</p>
                                <p style="margin:5px 0; color:#666; font-size:14px;"><strong>Email:</strong> {{ $booking->customer->email ?? 'N/A' }}</p>
                                <p style="margin:5px 0; color:#666; font-size:14px;"><strong>Phone:</strong> {{ $booking->customer->contact ?? 'N/A' }}</p>
                            </td>
                            <td style="vertical-align: top; width:50%; padding-left:15px; text-align:right;">
                                <p style="margin:5px 0; color:#666; font-size:14px;"><strong>Invoice No:</strong> {{ $booking->inv_no }}</p>
                                <p style="margin:5px 0; color:#666; font-size:14px;"><strong>Invoice Date:</strong> {{ $booking->created_at->format('F d, Y') }}</p>
                                <p style="margin:5px 0; color:#666; font-size:14px;"><strong>Payment Status:</strong>
                                    <span style="color:{{ $badgeColor }}; font-weight:bold;">{{ strtoupper($booking->payment_status) }}</span>
                                </p>
                            </td>
                        </tr>
                    </table>
                                            ...
                                            <td style="width:280px; vertical-align:top; text-align:center; padding-right:15px;">
                                                ...
                                                <div style="margin-top:8px;">
                                                    @foreach ($thumbnails as $thumb)
                                                        <img src="{{ $thumb ? asset('storage/' . ltrim($thumb, '/')) : 'https://via.placeholder.com/60x45?text=No+Image' }}" style="width:60px;height:45px;object-fit:cover;margin:0 3px;border-radius:3px;border:1px solid #ddd;">
                                                    @endforeach
                                                </div>
                                            </td>
                                            <td style="vertical-align:top; text-align:left; padding-left:15px;">
                                                <p style="margin:0 0 10px 0; font-size:18px; color:#333; font-weight:bold;">{{ $booking->vehicle->name }} - {{ $booking->vehicle->model }}</p>
                                                <p style="margin:8px 0; color:#666; font-size:14px;"><strong>Start Date/Time:</strong> {{ $booking->start_datetime?->format('Y-m-d H:i') ?? 'N/A' }}</p>
                                                <p style="margin:8px 0; color:#666; font-size:14px;"><strong>End Date/Time:</strong> {{ $booking->end_datetime?->format('Y-m-d H:i') ?? 'N/A' }}</p>
                                            </td>
                                        </tr>
                                    </table>
                                </td>
                    ...
                    <table style="width:100%; border-collapse:collapse; margin-bottom:20px; page-break-before: always;">
                        ...
                        <tbody>
                            <tr style="border-bottom:1px solid #ddd;">
                                ...
                                <td style="padding:12px; text-align:right; font-size:14px; color:#666;">{{ $booking->currency }} {{ number_format($booking->price,2) }}</td>
                            </tr>
                            <tr style="border-bottom:1px solid #ddd;">
                                <td style="padding:12px; font-size:14px; color:#666;">Additional Charges</td>
                                <td style="padding:12px; text-align:right; font-size:14px; color:#666;">{{ $booking->currency }} {{ number_format($booking->additional_price,2) }}</td>
                            </tr>
                            <tr style="border-bottom:1px solid #ddd;">
                                <td style="padding:12px; font-size:14px; color:#666;">Discount</td>
                                <td style="padding:12px; text-align:right; font-size:14px; color:#28a745;">- {{ $booking->currency }} {{ number_format($booking->discount,2) }}</td>
                            </tr>
                            <tr style="background:#f0f0f0;">
                                <td style="padding:15px 12px; font-size:16px; color:#333; font-weight:bold;">TOTAL AMOUNT</td>
                                <td style="padding:15px 12px; text-align:right; font-size:18px; color:#333; font-weight:bold;">{{ $booking->currency }} {{ number_format($booking->total_price,2) }}</td>
                            </tr>
                        </tbody>
                    </table>
